...
for(j in 1:length(mu1)){
  ...
  vec.wsr = numeric(R)
  for(i in 1:R){
    data = rexp(n, rate = mu1[j])
    
    #sign test
    k = ifelse(data>log(2), 1, 0); 
    s2 = sum(k)
    test.stat2 = (s2 - (n/2))/(sqrt(n/4))
    
    #wilcoxon sign rank test
    w.wsr = sum(k*rank(abs(data-log(2))))
    mean.wsr = (n*(n+1))/4
...
plot(mu1, emperical.power1, type = "l", main = "Power Curve", col = "blue")
lines(mu1, emperical.power.wsr, col = "red")


legend(0.2,0.3, legend = c("sign test", "WSR test"), fill = c("blue","red"))
...
legend(60,0.9980, legend = c("t-test", "sign test", "WSR test"), fill = c("blue", "green", "red"))
